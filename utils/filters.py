...
def aplicar_filtros(df, cookies):
    ...
    with st.container():
        ...
        # --- INÍCIO DA ALTERAÇÃO (Layout do Toggle/Botão) ---
        with col6:
            sub_col6_A, sub_col6_B = st.columns(2)
            
            with sub_col6_A:
                # 1. Label manual acima do toggle
                st.markdown(
                    "<label style='font-size: 1rem; color: #003366; font-weight: 600; margin-bottom: -10px;'>Rótulos de Dados</label>", 
                    unsafe_allow_html=True
                )
                st.toggle(
                    "Rótulos de Dados", 
                    key="filtro_show_labels", 
                    help="Exibir/Ocultar rótulos nos gráficos",
                    label_visibility="collapsed" # Esconde o label padrão
                )
            
            with sub_col6_B:
                # 2. Label manual acima do botão (para alinhar)
                st.markdown(
                    "<label style='font-size: 1rem; color: #003366; font-weight: 600; margin-bottom: -10px;'>Ações</label>", 
                    unsafe_allow_html=True
                )
                if st.button("Limpar Filtros", type="secondary", width='stretch'):
                    keys_to_delete = [k for k in st.session_state.keys() if k.startswith("filtro_")]
